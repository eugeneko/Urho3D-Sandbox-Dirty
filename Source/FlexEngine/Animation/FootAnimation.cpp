...
struct FootAnimationTrack
{
    ...
    FootAnimationKeyFrame SampleFrame(float time, unsigned& frame) const
    {
        ...
        result.kneeRotation_ = Quaternion(keyFrame->kneeRotation_).Slerp(Quaternion(nextKeyFrame->kneeRotation_), t).RollAngle();
...
void CreateFootAnimationTrack(FootAnimationTrack& track, Model* model, Animation* animation, const String& thighName)
{
    ...
    for (unsigned i = 0; i < numKeyFrames; ++i)
    {
        ...
        track.keyFrames_[i].kneeRotation_ = GetKneeRotationAngle(thighPosition, calfPosition, heelPosition, Vector3::FORWARD);
...
void FootAnimation::PostUpdate(float timeStep)
{
    ...
    FootAnimationTrack track;
    CreateFootAnimationTrack(track, model->GetModel(), animation_, footBoneName_);
    ...
    const FootAnimationKeyFrame frame = track.SampleFrame(time_, frameIndex);
    ...
    const Vector3 newCalfPosition = ResolveKneePosition(thighNode->GetWorldPosition(), newHeelPosition, forwardDirection,
        thighLength, calfLength, frame.kneeRotation_);
    ...
    thighNode->MarkDirty();
    time_ = Mod(time_ + timeStep, track.GetLength());
}
